...
- **Default style:** 2px border, 10px border-radius.
- **Color Logic:** The hint color is automatically derived from the system accent color (blue by default). Users can override this in GSettings, but the manual picker has been removed from the panel indicator to simplify the UI.
- **Rendering:** Uses standard CSS `border-radius` on the `St.Bin`. Legacy Cogl shader-based rounded corners were removed in v2.3.0 for better stability on GNOME 50.
- **Theming:** CSS loaded from `light.css`, `dark.css`, `highcontrast.css`.
- **Restack:** Each `ShellWindow` owns its own `_restack_id`. Restacking is deferred via `utils.later_add(BEFORE_REDRAW)` and cancelled/re-scheduled on rapid calls. This avoids corrupting stacking order.
...
| `pnpm run debug` | Build, install, and launch a nested Wayland session |

### Debugging Workflow

For GNOME 49 and 50, use the following standard nested session approach:
```bash
dbus-run-session gnome-shell --devkit --wayland
```
The `pnpm run debug` command automates this. The `--devkit` flag is essential for extension development in GNOME 50 to ensure the shell respects local extension paths and provides proper D-Bus access.
...
*Document Version: 2.3.1 | Updated: April 2026 (Reflected TSC migration & GNOME 50 cleanup)*
# O-tiling: Agent Documentation
